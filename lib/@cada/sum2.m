  ...
  for Vcount = 1:NUMvod
    if ~isempty(x.deriv(Vcount).nzlocs)
      ...
      elseif (Dim == 1 && xNcol == 1) || (Dim == 2 && xMrow == 1)
        ...
        if DPFLAG
          dx = sparse(xrows,xcols,1:nzx,xdim,nv);
        end
      elseif Dim == 1
        % x is a matrix, sum over first dimension
        dx = sparse(xrows,xcols,1:nzx,xMrow*xNcol,nv);
        dxR = reshape(dx,xMrow,xNcol*nv);
        [xRrows,xRcols] = find(dxR);
        yRcols = unique(xRcols(:));
        nzy = length(yRcols);
        yRrows = ones(nzy,1);
        dyR  = sparse(yRrows,yRcols,yRrows,1,yNcol*nv);
        ...
        xref = xref.';
        xrowst = xref(xrows);
        dxt  = sparse(xrowst,xcols,1:nzx,xMrow*xNcol,nv);
        dxtR = reshape(dxt,xNcol,xMrow*nv);
        [xtRrows,xtRcols] = find(dxtR);
        yRcols = unique(xtRcols(:));
        nzy = length(yRcols);
        yRrows = ones(nzy,1);
        dyR = sparse(yRrows,yRcols,yRrows,1,xMrow*nv);
        %dyR  = sum(dxtR,1);
